    ...
    public class splineMove : MonoBehaviour
    {
        ...
        private void Initialize(int startAt = 0)
        {
            ...
            for (int i = 0; i < wpPos.Length; i++)
                wpPos[i] = waypoints[i + startAt] + offset;
        ...
        private void CreateTween()
        {
            ...
            {
                if (waypointRotation != RotationType.none)
                {
            ...
            if (local)
            {
                tween = transform.DOLocalPath(wpPos, originSpeed, pathType, pathMode)
                                 ...
                                 .SetLookAt(lookAhead, forwardDir);
            }
            else
            {
        ...
        private void OnWaypointRotation()
        {
            ...
            if (local) nextPos = pathContainer.transform.TransformPoint(nextPos);
        ...
        /// <summary>
        /// Teleports to the defined time position on the path.
        /// </summary>
        public void GoTo(float t, bool includeLoop = true)
        {
            if (tween == null || t == 0f)
                return;
            ...
            if (reverse) t = tween.Duration(includeLoop) - t;
        ...
        public void ResetToStart()
        {
            ...
            if (pathContainer)
            {
                transform.position = pathContainer.waypoints[currentPoint].position + offset;
            }
        }
