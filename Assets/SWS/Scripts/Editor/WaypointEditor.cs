...
namespace SWS
{
    ...
    public class WaypointEditor : Editor
    {
        ...
        private int addPathPointNum = 10;
            ...
            else if (Event.current.keyCode == script.placementKey)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        float distance = 100f;
                        pos = worldRay.GetPoint(distance);
                    }

                    //place a waypoint at clicked point
                    if (pathMan is BezierPathManager)
                        PlaceBezierPoint(pos);
                    else
                        PlaceWaypoint(pos);
                }
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (placing)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Point Num: ", GUILayout.Height(15));
                addPathPointNum = EditorGUILayout.IntField(addPathPointNum, GUILayout.Height(15));
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);

                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("XFactor: ", GUILayout.Height(15));
                xFactor = EditorGUILayout.FloatField(xFactor, GUILayout.Height(15));
                GUILayout.Label("YFactor: ", GUILayout.Height(15));
                yFactor = EditorGUILayout.FloatField(yFactor, GUILayout.Height(15));
                if (GUILayout.Button("Sin", GUILayout.Width(100), GUILayout.Height(20)))
                {
                    PlaceSinPath();
                }
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("VSpeed: ", GUILayout.Height(15));
                vSpeed = EditorGUILayout.FloatField(vSpeed, GUILayout.Height(15));
                GUILayout.Label("WSpeed: ", GUILayout.Height(15));
                wSpeed = EditorGUILayout.FloatField(wSpeed, GUILayout.Height(15));
                if (GUILayout.Button("ArchiSpiral", GUILayout.Width(100), GUILayout.Height(20)))
                {
                    PlaceArchiPath();
                }
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("XLen: ", GUILayout.Height(15));
                xR = EditorGUILayout.FloatField(xR, GUILayout.Height(15));
                GUILayout.Label("YLen: ", GUILayout.Height(15));
                yR = EditorGUILayout.FloatField(yR, GUILayout.Height(15));
                if (GUILayout.Button("Oval", GUILayout.Width(100), GUILayout.Height(20)))
                {
                    PlaceOvalPath();
                }
                EditorGUILayout.EndHorizontal();
        ...
        void PlaceSinPath()
        {
            ...
            {
                Vector3 camPos = GetSceneView().camera.transform.position;
                Vector3 pos2D = camPos;
                pos2D.z = 0;

                for(int i = 0; i < addPathPointNum; i++)
                {
                    if (pathMan is BezierPathManager)
                        PlaceBezierPoint(pos2D);
                    else
                    ...
                    pos2D.x += 90f * xFactor;
                    angle += 90f;
                    pos2D.y += Mathf.Sin(angle * Mathf.Deg2Rad) * yFactor;
                }
            }
            else
            {
                Matrix4x4 m = GetSceneView().camera.cameraToWorldMatrix;
                Vector3 pos = GetSceneView().camera.ViewportToWorldPoint(new Vector3(0.5f, 0.5f, 50f));
                Vector3 offset;
                for (int i = 0; i < addPathPointNum; i++)
                {
                    if (pathMan is BezierPathManager)
                    ...
                    angle += 90f;
                    offset = new Vector3(90f * xFactor, Mathf.Sin(angle * Mathf.Deg2Rad) * yFactor, 0.0f);
                    offset = m * offset;
                    pos += offset;
                }
            }
        ...
        void PlaceArchiPath()
        {
            if (mode2D)
            {
                ...
                for (int i = 1; i <= addPathPointNum; i++)
                {
                    if (pathMan is BezierPathManager)
                        PlaceBezierPoint(pos2D);
                    else
                    ...
                    pos2D.x = camPos.x + vSpeed * i * Mathf.Cos(wSpeed * i);
                    pos2D.y = camPos.y + vSpeed * i * Mathf.Sin(wSpeed * i);
                }
            }
            else
            {
                Matrix4x4 m = GetSceneView().camera.cameraToWorldMatrix;
                Vector3 center = GetSceneView().camera.ViewportToWorldPoint(new Vector3(0.5f, 0.5f, 50f));
                Vector3 pos = center;
                Vector3 offset;
                for (int i = 1; i <= addPathPointNum; i++)
                {
                    ...
                    offset = new Vector3(vSpeed * i * Mathf.Cos(wSpeed * i), vSpeed * i * Mathf.Sin(wSpeed * i), 0.0f);
                    offset = m * offset;
                    pos = center + offset;
                }
            }
        ...
        void PlaceOvalPath()
        {
            float angle = Mathf.PI * 2f / addPathPointNum;
            ...
            if (mode2D)
            {
                ...
                for (int i = 0; i <= addPathPointNum; i++)
                {
                    pos2D.x = camPos.x + xR * Mathf.Cos(angle * i);
                    pos2D.y = camPos.y + yR * Mathf.Sin(angle * i);
            ...
            else
            {
                ...
                for (int i = 0; i <= addPathPointNum; i++)
                {
                    offset = new Vector3(xR * Mathf.Cos(angle * i), yR * Mathf.Sin(angle * i), 0.0f);
                    offset = m * offset;
                    pos = center + offset;
